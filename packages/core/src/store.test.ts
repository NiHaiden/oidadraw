...
import type { LineShape, RectShape } from "./types.js"
...
describe("local BoardStore", () => {
  it("keeps documents and undo history independent even with the same board ID", () => {
    const first = createStore()
    const second = createStore()

    first.putShape(node)
    first.setBoardName("First board")

    expect(second.getShapes()).toEqual([])
    expect(second.getBoardName()).toBe("")
    expect(second.getCanUndo()).toBe(false)
    expect(first.getCanUndo()).toBe(true)
  })

  it("notifies document subscribers and supports unsubscribing", () => {
    const store = createStore()
    const changed = vi.fn()
    const unsubscribe = store.subscribe(changed)

    store.setBoardName("Renamed")
    expect(changed).toHaveBeenCalledTimes(1)
    expect(store.getBoardName()).toBe("Renamed")
  ...
  it("separates undo groups through the public history command", () => {
    const store = createStore()
    store.putShape(node)
    store.stopCapturing()
    store.putShape({ ...node, x: 200 })
  ...
  it("destroys its document once without notifying disposed subscribers", () => {
    const store = createStore()
    store.putShape(node)
    const destroyed = vi.fn()
    const changed = vi.fn()
    store.doc.on("destroy", destroyed)
